...
int main(int argc, char const *argv[])
{
    ...
    for(int i = 0; i < n; i++){
        ...
        search_mp[0][title].insert(id);
        search_mp[1][author].insert(id);
        for(auto j : keywords)search_mp[2][j].insert(id);
        search_mp[3][publisher].insert(id);  
        search_mp[4][year].insert(id);
    }
    cin >> m;
    getchar();
    for(int i = 0; i < m; i++){
        ...
        if(search_mp[s_id][s].empty()){ cout << "Not Found" <<endl; continue; }
        for (auto j : search_mp[s_id][s]) printf("%07d\n",j);
    }
    return 0;
}