...
inline int cmp1(Player &a, Player &b) { return a.arrive < b.arrive; }
inline int cmp2(Player &a, Player &b) { return a.start < b.start; }
struct Table
{
...
static inline int findvip(int index, int before)
{
    for (int i = index; i < players.size() && players[i].arrive <= before; i++)
    {
        if (!players[i].served && players[i].isVIP)
            return i;
    }
    return -1;
}

static inline void update(int pla_id, int tb_id)
{
    players[pla_id].start = max(players[pla_id].arrive, tables[tb_id].endtime);
    players[pla_id].served = 1;
    tables[tb_id].endtime = players[pla_id].start + players[pla_id].use_time;
    tables[tb_id].serve_cnt++;
}
...
int main()
{
    ...
    for (int i = 0; i < players.size();)
    {
        ...
        if (21 * 3600 <= minEndTime || 21 * 3600 <= players[i].arrive)
            break;
        ...
        if (players[i].arrive <= minEndTime)
        {
            ...
            {
                for (int j = 0; j < m; j++)
                    if (tables[j].isVIP && tables[j].endtime <= players[pla_id].arrive)
                    {
                        tb_id = j;
                        break;
                    }
        ...
        else
        {
            for (int j = 0; j < m; j++)
            {
                if (tables[j].endtime <= players[pla_id].arrive)
                {
                    tb_id = j;
                    break;
                }
        ...
        update(pla_id, tb_id);
        while (i < players.size() && players[i].served)
            i++;
    }
